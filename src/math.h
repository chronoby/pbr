...
template <class T, int size>
Vector<T, size> Vector<T, size>::crossProduct(Vector<T, size>& rhs)
{
	T v[size] = { 0 };
	if (size != 3) {
		Vector<T, size> res(v);
	...
	double v1x = val[0], v1y = val[1], v1z = val[2];
	double v2x = rhs[0], v2y = rhs[1], v2z = rhs[2];
	return Vector<T, size>(v1y * v2z - v1z * v2y, v1z * v2x - v1x * v2z, v1x * v2y - v1y * v2x);
}
